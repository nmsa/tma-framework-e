...
class KeyManager:
    def decrypt(self, text, private_key_string):
        ...
        decryptedBytes = cipher.decrypt(text,"Error while decrypting")
        message = decryptedBytes.decode('UTF-8')
        return message
    ...
    def encrypt(self, text, public_key_string):
        ...
        ciphertextBytes = cipher.encrypt(bytes(text,"utf-8"))
    ...
    def sign(self, data, keyFile):
        ...
        encData = data.encode() #bytes(data,"utf-8")
        h = SHA.new(encData)
